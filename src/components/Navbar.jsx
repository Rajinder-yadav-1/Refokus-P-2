...
function Navbar() {
  return (
    <div className="max-w-screen-xl mx-auto py-6 p-5 flex justify-between items-center border-b-[1px] border-zinc-700">
      ...
      <h2 className="ml-2 text-xl">Refokus</h2>

      <div className="gap-14 flex ml-20 ">
        {["Home", `Works`, `Culture` , '' , 'News'].map((elem,index) => (
          
          elem.length === 0 ? <span className="h-7 w-[2px] bg-zinc-600"></span> : (
          <a className="text-m flex items-center gap-1.5" href="/">
            {index === 1 && (<span style={{boxShadow : "0 0 0.25em #00ff19"}} className="inline-block h-1 w-1 rounded-full bg-green-500"></span>)}
            {elem}
          </a>)
        ))}
      </div>
